        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            >
              ...
              Back to Exercise Planner
            </Link>
          </div>
          <Link to="/exercise-planner">
          ...
          <>
            {savedPlans.length === 0 ? (
              <div className="bg-white rounded-lg shadow-md p-8 text-center">
                <h3 className="text-xl mb-4">You don't have any saved plans yet</h3>
                <p className="mb-6 text-gray-600">Create your first fitness plan to get started on your journey</p>
                <Link to="/exercise-planner">
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <div className="flex items-center">
                          <h3 className="text-xl font-semibold">{plan.planName || `${plan.userData?.goal} Plan`}</h3>
                          <span className={`ml-3 px-2 py-1 text-xs rounded-full ${getDifficultyColor(plan.userData?.fitnessLevel)}`}>
                            {plan.userData?.fitnessLevel || 'Custom'}
                          </span>
                        </div>
                        <p className="text-gray-600 mt-1">{plan.planData?.summary || 'Custom workout plan'}</p>
                      </div>
                    ...
                    <div className="mb-4">
                      <div className="flex items-center space-x-4 text-sm text-gray-600">
                        <div>
                          <span className="font-medium">{plan.userData?.selectedDays?.length || 'N/A'} days</span> per week
                        </div>
                        <div>
                          Goal: <span className="font-medium">{plan.userData?.goal || 'Custom'}</span>
                        </div>
                        <div>
                          ...
                          >
                            ...
                            <div className="mt-auto">
                              {getActionButtons(
                                plan._id, 
                                index, 
                                workoutStatuses[plan._id]?.[index] || 'notStarted'
                              )}
                            </div>
                          </div>
                        ))}
