...
const FeatureCard = ({ title, description, link, icon }) => {
  return (
    <div className="h-full mb-4 text-center bg-white rounded-xl border border-blue-200 shadow-lg transition-all duration-300 hover:transform hover:-translate-y-2 hover:shadow-xl">
      <div className="p-4 flex flex-col h-full">
        {icon && (
          <div className="text-center mb-3">
            {React.cloneElement(icon, {
              size: 56,
              className: "mb-4 text-blue-600 mx-auto filter drop-shadow-lg",
            })}
          </div>
        )}
        <div className="h-[3px] w-10 bg-blue-600 mx-auto mb-5 rounded"></div>
        <h3 className="mt-2 mb-3 text-gray-900 font-semibold text-xl">{title}</h3>
        <p className="text-gray-600 flex-1 text-sm mb-5">{description}</p>
        <Link
          to={link}
          className="inline-block mt-auto bg-blue-600 border-none rounded-lg px-5 py-2.5 font-medium text-white transition-all duration-300 uppercase text-xs tracking-wider hover:bg-purple-600 hover:scale-105"
        >
          Learn More
        </Link>
      </div>
    ...
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <div className="container mx-auto py-12 px-4">
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4 inline-block relative drop-shadow-lg">
            FITNESS HYPER
            <span className="text-blue-600 relative drop-shadow-xl">
              {" "}
              FEATURES
            </span>
          </h1>
          ...
          <div className="h-1 w-20 bg-gradient-to-r from-blue-600 to-purple-600 mx-auto my-6 rounded shadow-md"></div>
        </div>
